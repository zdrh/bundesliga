...
if(! function_exists('form_open_rest')) {


    function form_open_rest(string $action, string $method,  $hidden, string $class, string $value, bool $inline = true) {
        ...
        if($inline == true) {
            ...
            $form = '<form action="' . $action . '"' . ' class="form-inline" method="post"'.">\n";    
        } else {
            
            $form = '<form action="' . $action . '"' . ">\n";    
        }
        if(is_array($hidden)) {
            foreach($hidden as $index => $row) {
                $form.= '<input type="hidden" name="'.$index.'" value="'.$row.'"/>'."\n";
            }
        }
        
        $form.= '<input type="hidden" name="_method" value="'.strtoupper($method).'"/>'."\n";
        $form.= '<button type="submit" class="btn '.$class.'">'.$value."</button>\n";
        $form.= '</form>';
...
if (! function_exists('form_input_bs')) {
     ...
     * @param mixed $data
     * @param mixed $extra
     * @param boolean $notation - jestli se mají před uvozovky přidávat \
     */
    function form_input_bs($data = '', string $bs = '', string $label = '', string $for= '', string $type = 'text', $notation = true): string
    {
        if($notation) {
            $quot = "\"";
            $endL = "\n";
        ...
        $return .= $tab.'<input class='.$quot.'form-control'.$quot.' ' . parse_form_attributes($data, $defaults, $quot) . " />".$endL."</div>".$endL;
        return $return;
    }
}

if (! function_exists('form_input_bs_class')) {
    /**
     * Text Input Field. If 'type' is passed in the $type field, it will be
     * used as the input type, for making 'email', 'phone', etc input fields.
     *
     ...
     * @param boolean $notation - jestli se mají před uvozovky přidávat \
     */
    function form_input_bs_class($data = '', string $bs = '', string $label = '', string $for= '', string $type = 'text', $notation = true): string
    {
        if($notation) {
            $quot = "\"";
            $endL = "\n";
        ...
        $return .= $tab.'<input ' . parse_form_attributes($data, $defaults, $quot) . " />".$endL."</div>".$endL;
        return $return;
    }
}

if (! function_exists('form_number_bs')) {
    /**
     * Text Input Field. If 'type' is passed in the $type field, it will be
     * used as the input type, for making 'email', 'phone', etc input fields.
     *
     ...
     * @param mixed $extra
     */
    function form_number_bs($data = '', string $bs = '', string $label = '', string $for= '', string $value = '', $extra = '', string $type = 'number'): string
    {
        $defaults = [
            'type'  => $type,
            'name'  => is_array($data) ? '' : $data,
        ...
        $return .= '<input class="form-control" ' . parse_form_attributes($data, $defaults) . stringify_attributes($extra) . " />\n</div>";
        return $return;
    }
}
...
if (! function_exists('form_dropdown_bs')) {
     ...
     * @param string $for - hodnota atributu for ve značce label
     ...
     * @param mixed $extra - dodatečné atributy ve značce select
     * 
     */
    function form_dropdown_bs($data = '', $options = [], string $bs = '', string $label = '', string $for= '', $selected = [], $disabled = [], $extra = '', $notation = true): string
    {
        ...
        $extra    = stringify_attributes($extra, false, $notation);
        
        $multiple = (count($selected) > 1 && stripos($extra, 'multiple') === false) ? ' multiple='.$quot.'multiple'.$quot : '';
        if($bs == '') {
            $form = "<div>".$endL;
        } else {
        ...
        if($label != '') {
            $form.= '<label for='.$quot.$for.$quot.' class='.$quot.'form-label'.$quot.'>'.$label."</label>".$endL;
        }

        
        $form     .= '<select class='.$quot.'form-select'.$quot . rtrim(parse_form_attributes($data, $defaults)) . $extra . $multiple . '>'.$endL;
        ...
        foreach ($options as $key => $val) {
            ...
            if (is_array($val)) {
                ...
                $form .= '<optgroup label='.$quot . $key .$quot. '\>'.$endL;
...
function form_dropdown_bs_class($data = '', $options = [], string $bs = '', string $label = '', string $for= '', $selected = [], $disabled = [], $extra = '', $notation = true): string
    {
        ...
        $form     .= '<select '  . rtrim(parse_form_attributes($data, $defaults)) . $extra . $multiple . '>'.$endL;
...
if (! function_exists('form_button')) {
     ...
     * @param mixed $extra
     */
    function form_button($data = '', $notation = '"'): string
    {
        $defaults = [
            'name' => is_array($data) ? '' : $data,
            'type' => 'button',
        ...
        return '<button ' . parse_form_attributes($data, $defaults, $notation) . '>'
                . $content
                . "</button>";
    }
}
...
if (! function_exists('parse_form_attributes')) {
     ...
     */
    function parse_form_attributes($attributes, array $default, $notation = '"'): string
    {
