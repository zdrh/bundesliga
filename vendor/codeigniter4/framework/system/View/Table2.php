...
class Table2
{
     ...
     * @return bool
     */
    public function setTemplate($template, $rowClass, $cellClass)
    {
        ...
        $this->template['row_class_start'] = $rowClass;
        $this->template['cell_class_start'] = $cellClass;
        return true;
    }
    ...
    public function generate($tableData = null)
    {
        ...
        if (! empty($this->rows)) {
            ...
            foreach ($this->rows as $key => $row) {
                ...
                if(array_key_exists($key, $this->template['row_class_start'] )) {
                    $out .= $this->template['row_class_start'][$key] . $this->newline;
                    $name = '';
                } else {
                    $name = fmod($i++, 2) ? '' : 'alt_';
                ...
                foreach ($row as $key2 => $cell) {
                    if($this->multiKeyExists($key, $key2, $this->template['cell_class_start'])) {
                        $temp = $this->template['cell_class_start'];
                    } else {
    ...
    private function multiKeyExists($key1, $key2, $array) {
        
        $result1 = array_key_exists($key1, $array);
        if($result1) {
            $array2 = $array[$key1];
            $result2 = array_key_exists($key2, $array2);
            $result = $result2;
        } else {
            $result = false;
        }
     ...
     * @return array
     */
    protected function _defaultTemplate($rowClass = array(), $cellClass = array())
    {
        return [
